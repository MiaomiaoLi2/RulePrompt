...
class EmbVerbalizer(Verbalizer):
    ...
    def __init__(self,
        ...
        super().__init__(tokenizer=tokenizer, classes=classes)    # get number of classes
    ...
    def from_file(self, select_num: Optional[int] = 40, truncate: Optional[float] = -1., dataset_name: Optional[str] = " ",
        ...
        class_names = [[' ' + item] for item in self.classes]
        sum_label_len = 0
        all_label_len = []
        all_label_names = []
...
class RuleVerbalizer(Verbalizer):
    r"""
    This is the implementation of knowledeagble verbalizer, which uses external knowledge to expand the set of label words.
    This class inherit the ``ManualVerbalizer`` class.
    Args:
    ...
    def __init__(self,
                 one_item_words: Optional[list] = None,
                 one_item_support: Optional[list] = None,
                 tokenizer: PreTrainedTokenizer = None,
                 model: Optional[PreTrainedModel] = None,
                 classes: Sequence[str] = None,
                 prefix: Optional[str] = " ",
        ...
        self.one_item_words=one_item_words
        self.one_item_support=one_item_support
        self.one_item_support_extend=[]
    ...
    def from_file(self, select_num: Optional[int] = 40, truncate: Optional[float] = -1., dataset_name: Optional[str] = " ",
        ...
        for i in range(len(class_names)):
            one_item_support_extend=[]
            for j in range(len(class_names[i])):
                one_item_support_extend.append(max(self.one_item_support[i]))#sum(self.one_item_support[i])/len(self.one_item_support[i])
            class_names[i].extend([" " + word.lstrip(" ") for word in self.one_item_words[i]])
            one_item_support_extend.extend(self.one_item_support[i])
            self.one_item_support_extend.append(one_item_support_extend)
